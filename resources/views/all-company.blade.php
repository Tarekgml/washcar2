...
@php
    $query = request('query');
    $companies = App\Models\User::where('role', 'company')
        ->where('price', '!=', 0)
...
@if($companies->isNotEmpty())
    <div class="container">
        @foreach($companies as $company)
            <div class="card" style="cursor: pointer;" onclick="window.location.href='{{ route('company-account', $company->id) }}';">
                <img src="/storage/{{ $company->profile_img }}" alt="{{ $company->name }}"  class="profile-img">
                <div class="user-info" style="margin-top:-10px">
                    <h3 style="font-size: 14px; font-family: Tajawal; margin-bottom:-10px;font-weight: 800;">{{ $company->name }}</h3>
                    <p style="font-family: Tajawal; margin-bottom:-5px">{{ $company->price }} د.ك</p>
                </div>
