...
<body>
    ...
    <form method="GET" class="search-form" style="display: inline;">
        <input type="text" name="query" class="search-input" placeholder="أبحث  " style="font-family: 'Tajawal', sans-serif;" value="{{ request('query') }}">
    </form>
    <div class="search-icon">
...
$firstImage = $images[0];
@endphp
            <div class="card" style="cursor: pointer;" onclick="window.location.href='{{ route('car-details', $company->id) }}';">
                <img src="/storage/{{ $firstImage }}" class="profile-img">
                <div style="margin-top:0px;margin-bottom:0px" class="user-info">
                    <h3 style="font-size: 14px; font-family: Tajawal;margin-top:0px;margin-bottom:5px; font-weight: 800;">{{ $company->name }}</h3>
                    <p style="font-family: Tajawal;margin-top:0px;margin-bottom:0px">{{ $company->price }} د.ك</p>
                </div>
                <div class="status" style="color:green;margin-top:0px;">
                    متاح الان
                </div>
            </div>
        @endforeach
