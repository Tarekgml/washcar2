...
<!--============================================المنتجات================================================-->
  ...
  @php
  $products = App\Models\Product::where('type', 'acc')->get();
  @endphp
  @if($products->isEmpty())
  <center>
      ...
      <h5 style="font-family: Almarai; text-align: center;">لا توجد إكسسوارات متاحة </h5>
  </center>
  @else
  @php
    $query = request('query');
    $companies = App\Models\Product::where('price', '!=', 0)
        ->where('type', 'acc')
    ...
    @foreach($companies as $company)
      <div class="card" style="cursor: pointer;" onclick="window.location.href='{{ route('Product-details', $company->id) }}';">
          ...
          <div class="user-info" style="margin-top: -10px;">
              <h3  style="font-size: 14px; font-family: Tajawal; margin-bottom: -20px; font-weight: 800;">{{ $company->name }}</h3>
              <p style="font-family: Tajawal; margin-bottom: -10px;">{{ $company->price }} د.ك</p>
          </div>
          <div class="status" style="color:green; margin-top: 2px;">
              متوفر
          </div>
      </div>
    @endforeach
